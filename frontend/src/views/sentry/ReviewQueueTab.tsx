...
export function ReviewQueueTab({ ctx }: { ctx: SentryContext }) {
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      <div className="flex items-center justify-between border-b border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-2 font-mono text-[10px]" style={{ letterSpacing: "0.1em" }}>
...
function ReviewColumn({
  ...
  return (
    <div className="flex w-1/3 flex-col overflow-hidden border-r border-[var(--color-border)] last:border-r-0">
      ...
      >
        <div className="flex items-center gap-2 font-mono text-[11px] font-semibold uppercase" style={{ color: accent, letterSpacing: "0.16em" }}>
          <span className="h-2 w-2 rounded-full" style={{ background: accent, boxShadow: `0 0 5px ${accent}` }} />
          {title}
          <span className="font-mono text-[11px] tabular-nums text-[var(--color-text-muted)]">({records.length})</span>
        </div>
        {bulkAction && records.length > 0 && (
          <button
            onClick={onBulk}
            disabled={bulkRunning}
            className="rounded-sm border border-[var(--color-border-active)] px-2 py-[2px] font-mono text-[10px] font-semibold uppercase text-[var(--color-text-secondary)] transition-colors hover:border-[var(--color-primary)] hover:text-[var(--color-primary)] disabled:opacity-40"
            style={{ letterSpacing: "0.16em" }}
          >
      ...
      <div className="flex-1 overflow-y-auto p-2">
        ...
        {records.length === 0 && (
          <div className="p-4 text-center font-mono text-[10px] text-[var(--color-text-muted)]" style={{ letterSpacing: "0.1em" }}>
            EMPTY
          </div>
        )}
      </div>
...
function ReviewCard({
  ...
  return (
    ...
    >
      ...
      <div
        className="flex items-center gap-2 border-b border-[var(--color-border)] px-2 py-1 font-mono text-[11px] font-semibold uppercase"
        style={{
      ...
      >
        ...
        {record.classification_discrepancy && (
          <span className="ml-auto rounded-sm border border-[var(--color-danger)] px-1 text-[9px]" style={{ color: "var(--color-danger)" }}>
            MIS-MARKED
          </span>
        )}
      </div>
      <div className="px-2 py-2">
        <div className="mb-1 flex items-center gap-2 font-mono text-[10px]" style={{ letterSpacing: "0.08em" }}>
        ...
        <div className="line-clamp-2 text-[11px] text-[var(--color-text-secondary)]">{record.remark}</div>
  ...
  return (
    <aside className="flex w-[28rem] shrink-0 flex-col overflow-y-auto border-l border-[var(--color-border)] bg-[var(--color-surface)]">
      ...
      <div className="flex flex-col gap-4 p-4">
        ...
        <div
          className="rounded-sm border-l-4 py-2 pl-3 pr-2 font-mono text-[13px] font-semibold uppercase"
          style={{
        ...
        >
          ...
          {record.classification_discrepancy && (
            <span className="ml-2 text-[10px] text-[var(--color-danger)]">MIS-MARKED</span>
          )}
        </div>
        ...
        <section>
          ...
          <div className="rounded-sm border border-[var(--color-border)] bg-[var(--color-bg)] p-3 font-mono text-[12px] leading-relaxed text-[var(--color-text)]">
            {segments.map((s, i) =>
              s.category ? (
                <span
                  key={i}
          ...
          <section>
            ...
            <div className="overflow-hidden rounded-sm border border-[var(--color-border)]">
              {sorted.map((h: any, i) => (
                <div
                  key={i}
                  className="flex items-center justify-between border-b border-[var(--color-border)] bg-[var(--color-bg)] px-2 py-1.5 text-[11px] last:border-b-0"
                >
                  <div className="flex items-center gap-2">
                    <span
                      className="rounded-sm border px-1 font-mono text-[9px] font-semibold uppercase"
                      style={{
        ...
        <section>
          ...
          <div className="flex items-center gap-2 rounded-sm border border-[var(--color-border)] bg-[var(--color-bg)] px-3 py-2 text-[12px]">
      ...
      <div className="sticky bottom-0 z-10 flex items-center gap-2 border-t border-[var(--color-border)] bg-[var(--color-surface)] p-3">
        <button
          onClick={onApprove}
          className="flex-1 rounded-sm border border-[var(--color-success)] bg-[color-mix(in_oklab,var(--color-success-muted)_30%,var(--color-surface))] px-3 py-2 font-mono text-[11px] font-semibold uppercase text-[var(--color-success)] hover:bg-[var(--color-success)] hover:text-white"
        ...
        <button
          onClick={onReject}
          className="flex-1 rounded-sm border border-[var(--color-danger)] bg-[color-mix(in_oklab,var(--color-danger-muted)_30%,var(--color-surface))] px-3 py-2 font-mono text-[11px] font-semibold uppercase text-[var(--color-danger)] hover:bg-[var(--color-danger)] hover:text-white"
          style={{ letterSpacing: "0.18em" }}
        >
          ✗ Reject (R)
        </button>
  ...
  return (
    <div className="max-h-[45%] shrink-0 overflow-y-auto border-t border-[var(--color-warning)] bg-[color-mix(in_oklab,var(--color-warning-muted)_12%,var(--color-surface))] p-3">
      <div className="mb-2 flex items-center justify-between">
        <div
          className="font-mono text-[11px] font-semibold uppercase text-[var(--color-warning)]"
        ...
        <button
          onClick={onClose}
          className="font-mono text-[10px] uppercase text-[var(--color-text-muted)] hover:text-[var(--color-text)]"
          style={{ letterSpacing: "0.16em" }}
        >
          Hide ✕
        </button>
          ...
          <thead>
            <tr>
              ...
              {equipTypes.map((e) => (
                <th
                  key={e}
                  className="p-1 text-left text-[9px] text-[var(--color-text-muted)]"
      ...
      <div className="mt-2 max-h-40 overflow-y-auto font-mono text-[10px]" style={{ letterSpacing: "0.04em" }}>
        {risks.map((r, i) => (
