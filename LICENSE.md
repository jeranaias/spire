...
| **ThermalHawk-Nano v2** | ~1.73M parameter thermal drone detection model (Thornveil proprietary architecture architecture, Thornveil training protocol-trained) with trained weights | Thornveil proprietary; provisional patent pending |
...
* Models trained during the event are gov't owned; the training methodology
  (HawkStack/Thornveil training methodology) used to produce them is Thornveil IP.
