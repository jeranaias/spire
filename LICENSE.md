...
| **HawkStack** | Neural architecture methodology, recipe generator CLI (`thornveil-recipe-cli`), Thornveil training protocol training protocol, and 15-checkpoint model zoo | Thornveil proprietary; provisional patent pending |
| **ThermalHawk-Nano v2** | Thermal drone detection model family (Thornveil proprietary architecture architecture, SGDR-trained) with trained weights. Original capability-tier v2 variant (val mAP 82.95% on Anti-UAV410) and capability-tier Thornveil proprietary architecture variant (val mAP 82.12%) — the variant currently deployed in SPIRE BASTION live inference. | Thornveil proprietary; provisional patent pending |
