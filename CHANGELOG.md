...
- **Deployment bundle** — multi-stage `Dockerfile` (Node 24 + pnpm) with
  `marlog-api` and `marlog-web` targets, root `docker-compose.yml` covering
  API / web / Postgres for local stand-up, `fly.marlog.toml` and
  `fly.marlog-web.toml` Fly.io companions to SPIRE's `fly.toml`,
  `deploy/nginx.conf` for the SPA + `/api` proxy, and `deploy/FLY_DEPLOY.md`
  documenting the end-to-end deploy procedure.
- **CI workflow** — `.github/workflows/ci.yml` runs `pnpm run build`
  (typecheck + per-package build) and builds both Docker images on every push
  and pull request.
