...
import { seedSupplyHistoryFromResupplyEvents } from "./lib/seed-supply-history";
import { startCommsHygieneScheduler } from "./lib/comms-hygiene";
...
app.listen(port, (err) => {
  ...
  startCommsHygieneScheduler();
  void seedSupplyHistoryFromResupplyEvents().catch((err) => {
    logger.error({ err }, "Supply history seed failed");
  });
});
