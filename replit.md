...
- **PULSE-Risk Holdout MAE (Task #87)**: Published with named baseline (`FY24 G-4 SOP heuristic`) and frozen holdout window (2026-03-04 → 2026-04-26, n=352). Backend implements `_holdout_mae()` with 1000-iter percentile bootstrap CI (seed=42). Reproducibility via `scripts/pulse_baseline_eval.py`. Model card at `docs/PULSE_RISK_MODEL_CARD.md` and slide 5 of pitch deck (`frontend/src/views/pitch/slides.ts`) cite these numbers.
