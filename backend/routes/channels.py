...
SUPPORTED_TYPES = {"filesystem", "sftp", "imap"}
...
async def channel_health_endpoint_alt(channel_id: str, request: Request):
    """Same as channel_health_endpoint below; route registered
...
async def poll_channel_endpoint_alt(channel_id: str, request: Request):
    """Same as poll_channel_endpoint below; route registered before
    the generic /{channel_id:path} routes so the :path matcher
    doesn't swallow the /poll suffix."""
