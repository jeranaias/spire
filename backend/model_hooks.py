...
Loading semantics:
  ...
  - ThermalHawk: presence-only by default — we record that the weights
    ...
    Thornveil-licensed-param model would dominate cold-start). When SPIRE_THERMALHAWK_EAGER=1
...
class ModelState:
    ...
    def _detect_thermalhawk(self) -> None:
        ...
        try:
            ...
            # (these are the published numbers for the Thornveil-licensed v2).
            self.thermalhawk_metadata = {
                ...
                "validation_map_50_95": 0.8295,  # (accuracy) — paper figure
            }
        except Exception as e:  # noqa: BLE001
    ...
    def _maybe_eager_thermalhawk(self) -> None:
        ...
        backend.ml.thermalhawk so we don't depend on the external repo
        ...
        if not self.thermalhawk_path:
            return
        try:
            from .ml.thermalhawk import load_thermalhawk
        except Exception as e:  # noqa: BLE001
            self.errors.append(f"ThermalHawk vendored arch import failed: {e}")
            return
        try:
            model = load_thermalhawk(self.thermalhawk_path, device="cpu")
