          ...
          {result ? (
            <Card className="border-border">
              <CardHeader className="border-b border-border pb-3 pt-4 px-4 flex flex-row items-center justify-between">
                ...
                <Button variant="outline" size="sm" className="font-mono uppercase text-[10px] tracking-widest h-7 px-2" onClick={() => window.print()}>
                  ...
                  {result.lines.map((line, i) => (
                    <div key={i} className="px-4 py-3 flex items-center justify-between hover:bg-muted/20 transition-colors">
                      ...
                      <div className="text-right">
                        ...
                        <div className="font-mono text-[10px] text-muted-foreground tabular-nums">{(line.dailyConsumption * result.personnel).toFixed(2)}/day</div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="h-full min-h-[400px] border border-dashed border-border/50 rounded-sm flex flex-col items-center justify-center text-muted-foreground bg-card/30">
              <Calculator className="w-10 h-10 mb-4 opacity-10" />
              <p className="font-mono text-xs tracking-widest uppercase">Run calculation to generate bill of materials</p>
            </div>
          )}
        </div>
      </div>
