...
For contributions to the initial MARLOG drop into SPIRE, branch from `master`
and PR against `master` (following SPIRE's contribution target for additive
sibling modules). For ongoing feature work after MARLOG lands, branch from
`develop` and PR back to `develop` per SPIRE's standard workflow. Use the
prefix `marlog/` for your branch name, e.g. `marlog/fix-dos-calculation`.
...
The API server has a Vitest integration test suite that exercises every
route file against the dev database. Push the schema first if it's stale,
then run:
...
The suite uses `DATABASE_URL` from the environment and writes/cleans up
isolated test rows — it is safe to run against a shared dev database.
